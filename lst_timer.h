...
public:
    ...
    void add_timer(util_timer *timer) {
        ...
        // 作为链表新的头节点，否则就需要调用重载函数add_timer(),
        // 把它插入链表中合适的位置，以保证链表的升序特性
        if (timer->expire < head->expire)  // 插到头部
        {
            timer->next = head;
            head->prev = timer;
            head = timer;
        ...
        add_timer(timer, head);  // 否则找到该插的位置
    }
    ...
    void adjust_timer(util_timer *timer) {
        ...
        // 如果被调整的目标定时器处在链表的尾部，或者该定时器新的超时时间值仍然小于其下一个定时器的超时时间
        // 则不用调整
    ...
    void tick() {
        ...
        // 由于定时器是升序，故遇到第一个没到期的，后面的所有都没到期
        while (cur != nullptr) {
            /* 因为每个定时器都使用绝对时间作为超时值，所以可以把定时器的超时值和系统当前时间，
            比较以判断定时器是否到期*/
            if (deadline < cur->expire) {
                break;
            }
